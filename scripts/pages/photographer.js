...
async function getPhotographeData() {
    ...
    fiches.media.filter(m => m.photographerId == photographe.id).forEach(currentMedia => {
        const card = document.createElement("section");
        media.appendChild(card);
       mediaTemplate(currentMedia,photographe.name).displayMedia(card);
    ...
    function updateMediaDisplay(sortedMedia) {
        ...
        sortedMedia.forEach(currentMedia => {
            const card = document.createElement("section");
            media.appendChild(card);
            const mediaModel = mediaTemplate(currentMedia).displayMedia(card);
        });
    }
